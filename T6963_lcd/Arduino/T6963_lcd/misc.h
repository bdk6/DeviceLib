...
static uint8_t CG_offset;

// Are we using 6 or 8 bit wide font?
static uint8_t fontSize = 6;
...
void T6963_setGraphicArea(uint8_t cols)
{
  ...
  // TODO: do we need a variable to track this?
}
...
void T6963_printChar(char c)
{
  ...
  T6963_setCursor(cursorX, cursorY);
//  T6963_gotoXY(cursorX, cursorY);
}
...
{
  // set address
  T6963_setAddress(textBaseAddress);
  T6963_setAutoWrite();
...
{
  T6963_setAddress(graphicBaseAddress);
  T6963_setAutoWrite();
  int bytes = width * height * 8; // 8 should be font size
  for(int cnt = 0; cnt < bytes; cnt++)
  {
...
static void T6963_gotoXY(uint8_t x, uint8_t y)
{
  if(x < width && y < height)
  {
    ...
    wait();
    T6963_writeCommandByte(T6963_SET_CURSOR_POINTER);
    T6963_setAddress(textBaseAddress + y * width + x);
  }
}
...
//  T6963_setAutoWrite();
//  for(int i =2000; i < 4560; i++)
//  {
//    T6963_writeDataByte( 0x57);
//  }
//  T6963_setAutoReset();
//  for(int i = 0; i <20; i++)
//  {
